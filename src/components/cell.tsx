...
export const Cell = memo(function Cell<T>({ itemId, rowIndex }: { itemId: Id; rowIndex: number }) {
  ...
  const column = state.useState((state) => state.activeColumns.find((column) => column.id === columnId));
  const item = state.useState((state) => state.activeItemsById.get(itemId));
  const css = state.useState((state) => state.theme.css);
  const columnStyleOverride = state.useState((state) => state.columnStyleOverride.get(columnId), { throttle: 16 });

  if (!column || !item) return null;
  state.getState().props.debugRender?.('render cell', itemId, columnId);
